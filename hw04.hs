...
-- 4. Реализуйте функции length, elemByIndex и elemIndices.
--    Все функции должны быть хвосторекурсивными.

-- | Функция, которая находит длину заданного списка. (0.5 б)
--
length :: [a] -> Int
length = undefined

-- | Функция, которая выдаёт элемент списка по его индексу.
--   Если индекс лежит за границами списка, то в результате нужно вернуть Nothing. (1 б)
--
elemByIndex :: [a] -> Int -> Maybe a
elemByIndex = undefined

-- | Функция, которая выдаёт позиции заданного целого числа в списке. (1 б)
--
elemIndices :: [Int] -> Int -> [Int]
elemIndices = undefined

-- | Бонусное задание!!! В том плане, что его невыполнение не может ухудшить вашу успеваемость.
--   Реализуйте функцию chunksOf, которая будет разбивать список на подсписки заданного размера. (1 б)
--
chunksOf :: [a] -> Int -> [[a]]
chunksOf = undefined

-- 5. Реализуйте функцию reverseLinear, которая будет разворачивать список.
